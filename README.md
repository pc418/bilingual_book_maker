...
A scan needs `--pdf-ocr` (and `--ocr-lang iso:ja` or the like outside Chinese and English); a paper gains from `--img-model gpt-5.6-luna`. Without `--to-epub` a PDF becomes a bilingual `.txt`. Read more: [PDF to bilingual EPUB](docs/features/pdf-to-epub.md) and [Recommended settings for PDF](docs/features/recommended-pdf.md).
