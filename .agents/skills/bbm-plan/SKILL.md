...
| `--accumulated_num` | integer (tokens per request) | *unset* — only runs of short lines share a request, **except session mode, where unset means `800`** | a long prose book where request count is the cost driver: consecutive units of any length then share one request up to N tokens. `800` is the knee of the savings curve; `--use_context session` already defaults there, and an explicit `1` is the off switch in any mode. Interrupted runs checkpoint and `--resume` either way |
