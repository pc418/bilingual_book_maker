...
| `--ocr-lang LANGS` | PDF only, with `--to-epub --with-ocr`: the languages the OCR models read on pages with no text layer, as EasyOCR codes, comma separated (`ch_sim,en`, `ch_tra`, `ja`, `ko`; list at https://www.jaided.ai/easyocr/). The default is `en,es,fr,de`, so a scan in another script needs the flag; the run says so when it meets a scanned page without it. An unknown code is refused before any page is read. |
