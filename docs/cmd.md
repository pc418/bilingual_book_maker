...
| `--classify-model MODEL` | Model for every classification step (old name `--plan-classify-model`); implies model mode on an epub; with `agent` its session is asked. Default: the provider entry's `classify_model`, else the run's model. |
