...
| `--structure-model MODEL` | PDF only, with `--to-epub`: a vision model on this endpoint that corrects the layout detector's region roles (text, heading, title, caption, footnote, code) from a page image before export; off by default. Needs an endpoint that accepts image input; the detector's labels stand wherever the model abstains or the endpoint cannot see the page. About 3k prompt tokens per page. |
...
| `--ocr-lang LANGS` | PDF only, with `--to-epub --pdf-ocr`: the languages the OCR engine reads on pages with no text layer, comma separated, as portable `iso:` tags (`iso:zh-Hans`, `iso:zh-Hant`, `iso:ja`, `iso:ko`) or the engine's own codes. The engine is the one docling selects on the install (rapidocr with the shipped requirements: default Chinese and English, one language per run) and the run prints the engine and languages it used. A scan in another script needs the flag; the run says so when it meets a scanned page without it. An unknown code is refused before any page is read. |
