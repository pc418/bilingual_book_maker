...
| `--img-key KEY` | Key for `--img-base-url`; defaults to the run's key on the same endpoint, else the endpoint format's environment variable. |
...
| `--ocr-lang LANGS` | PDF only, with `--to-epub --pdf-ocr`: the languages the OCR engine reads on pages with no text layer, comma separated, in the engine's own codes (rapidocr: `ch`, `en`, `latin`; easyocr: `ch_sim`, `ja`, `ko`; ocrmac: `zh-Hans`, `ja-JP`), or portable BCP-47 tags behind `iso:` (`iso:zh-Hans`, `iso:zh-Hant`, `iso:ja`, `iso:ko`), which docling 2.129 maps onto whichever engine runs. rapidocr reads one language per run and uses the first. The engine is the one docling selects on the install (rapidocr with the shipped requirements, whose default reads Chinese and English), and the run prints the engine and languages it used. A scan in another script needs the flag; the run says so when it meets a scanned page without it. An unknown code is refused before any page is read. |
