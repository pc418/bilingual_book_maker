...
| `--classify-key KEY` | Key for `--classify-base-url`; same default rule as `--img-key`. A Jev host's own variable (`JEV_API_KEY`/`TYPESAFE_API_KEY` at typesafe.ai, `FEATHERLESS_API_KEY` at featherless.ai) is read only at that host. |
