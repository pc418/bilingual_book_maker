...
=== "Novel"
    ...
    If a recurring name drifts after a window seam, pin it with a glossary file:
...
=== "Textbook with tables and formulas"
    ...
    A textbook has terms that must hold across hundreds of pages. Pin them.
    ...
    For a textbook that is a PDF, see [PDF to bilingual EPUB](pdf-to-epub.md); session mode works there the same way.

=== "Paper"

    A PDF paper extracts into many short blocks. A session gives each block the paragraphs before it.
...
A hosted endpoint needs the same command on every system. The system matters when the model runs locally, where a session costs time instead of money.

=== "macOS (Apple silicon)"

    A local model with a small context: bound the window to it.
...
=== "CPU only"
    ...
    Re-reading a history on every request is slow on a CPU model. Use a hosted endpoint with prompt caching:
