...
- `--context-compact-at`:
  ...
  `--context-compact-at 0` 改为按模型自身的上下文窗口来定预算：取窗口的 90%（用 `--model_list` 时取其中最小的那个窗口），在第一次付费请求之前问一次。端点报不出窗口时直接结束运行，而不是退回某个数字——那个数字恰恰是对"谁也量不出的那个模型"的猜测。只有 OpenAI 形状的端点能被问到，其他格式必须给具体数字。
