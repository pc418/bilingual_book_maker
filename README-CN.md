...
扫描件需要 `--pdf-ocr`（中英文以外的语言再加 `--ocr-lang iso:ja` 之类）；论文加 `--img-model gpt-5.6-luna` 效果更好。不加 `--to-epub` 时，PDF 输出为双语 `.txt`。详见：[PDF 转双语 EPUB](docs/features/pdf-to-epub.md) 和 [PDF 推荐设置](docs/features/recommended-pdf.md)。
